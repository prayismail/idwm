...
function fetchWeatherData(lat, lon) {
    ...
    let isGabungan = model === 'gabungan';
    
    // Jika gabungan, kita panggil semua model sekaligus dalam satu URL
    let allModels = "ecmwf_ifs,icon_seamless,ukmo_seamless,gfs_seamless,cma_grapes_global,meteofrance_seamless";
    let modelsParam = isGabungan ? allModels : model;

    // PERUBAHAN: timezone=auto diubah menjadi timezone=GMT agar data yang ditarik adalah murni UTC
    fetch(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&hourly=precipitation,wind_speed_10m,wind_direction_10m&models=${modelsParam}&current=precipitation,wind_speed_10m,wind_direction_10m&timezone=GMT&forecast_days=3&wind_speed_unit=kn`)
        .then(response => response.json())
        .then(data => {
            ...
            if (isGabungan) {
                // LOGIKA GABUNGAN MODEL (HANYA HUJAN - LINE CHART)
                let combinedDatasets = [];
                let modelNames = {
                    'ecmwf_ifs': 'IFS-ECMWF',
                    'icon_seamless': 'ICON-DWD',
                    'ukmo_seamless': 'UK-MetOffice',
                    'gfs_seamless': 'GFS-NCEP',
                    'cma_grapes_global': 'GRAPES-CMA',
                    'meteofrance_seamless': 'ARPEGE-MFI'
                };
                
                // Warna beda untuk setiap model agar mudah dibedakan di line chart
                let colors = ['#e6194B', '#3cb44b', '#4363d8', '#f58231', '#911eb4', '#000000'];
                let i = 0;
                
                for (let m in modelNames) {
                    let precipKey = `precipitation_${m}`;
                    if (hourly[precipKey]) {
                        combinedDatasets.push({
                            label: modelNames[m],
                            data: hourly[precipKey],
                ...
                currentData = { times, isGabungan: true, datasets: combinedDatasets };
                fetchElevation(lat, lon, times, null, null, null, combinedDatasets);
                
            } else {
                // LOGIKA MODEL TUNGGAL (SEPERTI SEMULA: BAR HUJAN + LINE ANGIN)
                let precipitation = hourly.precipitation;
                let windSpeed = hourly.wind_speed_10m;
                let windDirection = hourly.wind_direction_10m;
                
                currentData = { times, precipitation, windSpeed, windDirection, isGabungan: false };
                fetchElevation(lat, lon, times, precipitation, windSpeed, windDirection, null);
            }
        })
        .catch(error => {
...
function fetchElevation(lat, lon, times, precipitation, windSpeed, windDirection, combinedDatasets) {
    fetch(`https://api.open-meteo.com/v1/elevation?latitude=${lat}&longitude=${lon}`)
        ...
        .then(data => {
            ...
            // Melempar combinedDatasets ke showChart
            showChart(times, precipitation, windSpeed, windDirection, combinedDatasets);
        })
        .catch(error => {
...
function showChart(times, precipitation, windSpeed, windDirection, combinedDatasets) {
    chartPopup.style.display = 'block';
    let ctx = document.getElementById('weatherChart').getContext('2d');
    if (typeof weatherChart !== 'undefined' && weatherChart) weatherChart.destroy();
    
    let isGabungan = combinedDatasets !== null;
    let chartDatasets = [];
    
    if (isGabungan) {
        chartDatasets = combinedDatasets;
    } else {
        chartDatasets = [
            {
    ...
    // Mengatur Axis Y (Jika gabungan, Y2 untuk angin disembunyikan agar grafik rapi)
    let scalesConfig = {
        x: { 
            title: { display: true, text: 'Waktu (UTC)' } 
        },
        y1: { 
    ...
    if (!isGabungan) {
        scalesConfig.y2 = { 
            ...
            title: { display: true, text: 'Angin'} 
        };
    }

    let subtitleText = isGabungan 
        ? `Prakiraan cuaca oleh Open-Meteo. Perbandingan Ensemble Model` 
        : `Prakiraan cuaca oleh Open-Meteo. Berdasarkan model: ${modelSelect.options[modelSelect.selectedIndex].text}`;

    weatherChart = new Chart(ctx, {
        type: 'bar', // Base type, akan di-override di setiap dataset
        data: {
            // Memaksa JS membacanya sebagai UTC (dengan menambah 'Z' pada akhir teks waktu)
            labels: times.map(t => new Date(t + 'Z').toLocaleString('id-ID', { 
                timeZone: 'UTC', 
                hour: '2-digit', 
                minute: '2-digit', 
                day: 'numeric', 
                    ...
                    navPointsGeoJsonLayer = L.geoJSON(data, {
                        ...
                        onEachFeature: function(feature, layer) {
                            if (feature.properties && feature.properties.name_rep) {
                                layer.bindPopup(`<strong>${feature.properties.name_rep}</strong><br>Tipe: ${feature.properties.type_rep}`);
                            }
                        }
                    });
...
const vaacApiUrl = 'https://idwm-vaac.vercel.app/api/check-vaac'; // Pastikan URL API Anda benar
const debugStatusElement = document.getElementById('va-debug-status');
// Data Poligon WAAF untuk Double Protect di Frontend
const firUPG_geojson_frontend = {
    "geometry": { 
        "coordinates": [[[140.99, -6.32], [141, -6.32], [141, 3.5], [133, 3.5], [132.53, 4], [118, 4], [117.89, 4.18], [117.68, 4.17], [117.61, 4.14], [117.54, 4.17], [117.53, 4.16], [117.48, 4.17], [117.44, 4.19], [117.43, 4.23], [117.39, 4.26], [117.37, 4.29], [117.31, 4.3], [117.25, 4.37], [117.23, 4.37], [117.2, 4.34], [117.15, 4.35], [117.11, 4.33], [117.04, 4.35], [117.02, 4.32], [117.01, 4.32], [117.01, 4.35], [116.97, 4.34], [116.91, 4.37], [116.84, 4.33], [116.82, 4.35], [116.79, 4.34], [116.75, 4.39], [116.71, 4.33], [116.67, 4.35], [116.63, 4.34], [116.61, 4.38], [116.59, 4.37], [116.56, 4.41], [116.52, 4.33], [116.49, 4.33], [116.48, 4.3], [116.44, 4.29], [116.43, 4.33], [116.35, 4.39], [116.28, 4.36], [116.17, 4.39], [116.17, 4.33], [116.12, 4.34], [116.07, 4.28], [116.03, 4.3], [116.03, 4.33], [116, 4.35], [115.93, 4.35], [115.91, 4.39], [115.88, 4.39], [115.86, 4.34], [115.87, 4.29], [115.83, 4.26], [115.82, 4.23], [115.8, 4.22], [115.79, 4.24], [115.77, 4.24], [115.73, 4.19], [115.7, 4.19], [115.68, 4.16], [115.64, 3.96], [115.62, 3.94], [115.59, 3.94], [115.56, 3.92], [115.58, 3.88], [115.62, 3.87], [115.61, 3.82], [115.58, 3.75], [115.57, 3.65], [115.58, 3.6], [115.61, 3.55], [115.61, 3.51], [115.66, 3.44], [115.62, 3.41], [115.61, 3.45], [115.59, 3.45], [115.54, 3.36], [115.55, 3.33], [115.53, 3.32], [115.52, 3.25], [115.53, 3.23], [115.52, 3.22], [115.52, 3.19], [115.56, 3.17], [115.56, 3.15], [115.54, 3.14], [115.51, 3.11], [115.52, 3.06], [115.5, 3.03], [115.44, 3.02], [115.4, 2.98], [115.33, 2.98], [115.32, 3.01], [115.28, 3.05], [115.25, 2.97], [115.16, 2.93], [115.15, 2.91], [115.15, 2.87], [115.09, 2.82], [115.15, 2.79], [115.14, 2.75], [115.09, 2.7], [115.11, 2.69], [115.12, 2.65], [115.09, 2.61], [115.11, 2.58], [115.18, 2.61], [115.22, 2.54], [115.25, 2.54], [115.25, 2.51], [115.2, 2.47], [115.14, 2.48], [115.12, 2.45], [115.1, 2.4], [115.05, 2.4], [115.05, 2.39], [115.04, 2.39], [115.03, 2.36], [115, 2.35], [114.96, 2.37], [114.95, 2.32], [114.97, 2.29], [114.92, 2.26], [114.87, 2.26], [114.83, 2.25], [114.82, 2.26], [114.79, 2.2], [114.74, 2.19], [114.74, 2.14], [114.8, 2.15], [114.82, 2.13], [114.8, 2.09], [114.81, 2.06], [114.79, 2.06], [114.81, 2.02], [114.86, 2.04], [114.89, 2.02], [114.85, 1.96], [114.88, 1.91], [114.82, 1.89], [114.79, 1.85], [114.75, 1.87], [114.72, 1.86], [114.73, 1.83], [114.69, 1.81], [114.72, 1.78], [114.71, 1.64], [114.65, 1.59], [114.61, 1.57], [114.6, 1.53], [114.62, 1.52], [114.59, 1.45], [114.52, 1.44], [114.5, 1.48], [114.47, 1.48], [114.41, 1.52], [114.38, 1.52], [114.38, 1.49], [114.33, 1.48], [114.3, 1.46], [114.24, 1.45], [114.21, 1.41], [114.14, 1.47], [113.97, 1.45], [113.92, 1.41], [113.82, 1.37], [113.83, 1.34], [113.8, 1.3], [113.7, 1.27], [113.67, 1.22], [113.63, 1.22], [113.62, 1.25], [110.38, -3], [110.38, -8.33], [114.5, -12], [123.33, -12], [126.83, -9.33], [135, -7], [139.67, -9.83], [141, -9.83], [141.03, -9.62], [141.02, -6.89], [140.99, -6.9], [140.97, -6.89], [140.95, -6.91], [140.94, -6.89], [140.95, -6.86], [140.94, -6.87], [140.92, -6.87], [140.9, -6.84], [140.92, -6.82], [140.87, -6.79], [140.9, -6.75], [140.85, -6.72], [140.85, -6.68], [140.87, -6.68], [140.86, -6.66], [140.87, -6.65], [140.86, -6.64], [140.88, -6.62], [140.84, -6.61], [140.9, -6.59], [140.91, -6.57], [140.89, -6.57], [140.89, -6.56], [140.94, -6.56], [140.92, -6.55], [140.92, -6.5], [140.96, -6.49], [140.95, -6.47], [140.95, -6.45], [140.92, -6.47], [140.94, -6.44], [140.92, -6.42], [140.95, -6.43], [140.94, -6.41], [140.98, -6.39], [140.94, -6.38], [140.98, -6.36], [140.96, -6.35], [140.96, -6.33], [140.99, -6.32]]] 
    }
};
// --- VARIABEL ARSIP ---
// Mengambil arsip dari local storage, atau membuat array kosong jika belum ada
let vaaArchive = JSON.parse(localStorage.getItem('vaaArchive')) || [];
let archiveControl = null;

// --- MEMBUAT KOTAK PANEL ARSIP LEAFLET ---
...
// Fungsi Ray-Casting Algorithm di Frontend
...
function generateSigmet(vaaFullText, seqNumber = 'XX') { 
    ...
    function calculateIntensity(text) {
        ...
        // Jika prediksi poligon mengecil lebih dari 25% atau hilang
        if (fcstPts.length === 0 || fcstArea < obsArea * 0.75) return "WKN";
        // Jika prediksi poligon membesar lebih dari 25%
        if (fcstArea > obsArea * 1.25) return "INTSF";
...
// Fungsi untuk menyimpan VAA baru ke dalam arsip
function saveToArchive(vaaData) {
    // Cek apakah data ini sudah ada di dalam arsip berdasarkan hash-nya
    const exists = vaaArchive.find(item => item.advisoryHash === vaaData.advisoryHash);
    
    if (!exists) {
        // Beri stempel waktu saat ini
        vaaData.fetchTime = Date.now();
    ...
    const 24HoursAgo = Date.now() - 86400000;
    vaaArchive = vaaArchive.filter(item => item.fetchTime > 24HoursAgo);

    // Simpan permanen ke memori browser
    localStorage.setItem('vaaArchive', JSON.stringify(vaaArchive));
    renderArchiveList(); // Update tampilan daftar
}

// Fungsi untuk menggambar daftar VAA di kotak HTML
function renderArchiveList() {
    const listEl = document.getElementById('vaa-archive-list');
    if (!listEl) return;
 ...
 */
function showVaaNotificationOnMap(vaaData, isFromArchive = false) {
    ...
    // HANYA bunyikan alarm jika ini BUKAN dari panel arsip
    if (alertSound && !isFromArchive) {
...
async function checkForNewVAA() {
    ...
    if (data.advisoryHash !== lastAdvisoryData.advisoryHash) {
        ...
        // --- FILTER 1 (Backend): Cek penolakan dari Cloudflare ---
        // (Disesuaikan dengan pesan penolakan yang baru dari backend)
        if (data.message && data.message.includes("Outside FIR Ujung Pandang")) {
            console.log("[VAA] VAA diabaikan dari Backend: " + data.message);
            updateDebugStatus(`Status OK. VAA ditolak oleh backend.`);
            return; // <-- Skrip berhenti, abaikan notifikasi
        }

        // --- FILTER 2 (Frontend): Pengecekan GeoJSON WAAF Langsung di Browser ---
        const mapInfo = parseVaaForMapInfo(data.fullText);
        if (mapInfo) {
            const isInsideWAAF = isPointInPolygon(
                [parseFloat(mapInfo.lon), parseFloat(mapInfo.lat)], 
            ...
            // JIKA DI LUAR WAAF (seperti KRAKATAU), BATALKAN MUNCULNYA POPUP!
            if (!isInsideWAAF) {
                console.log(`[VAA] Gunung ${mapInfo.volcanoName} (${mapInfo.lat}, ${mapInfo.lon}) DIABAIKAN karena DI LUAR batas FIR Ujung Pandang.`);
                updateDebugStatus(`Status OK. ${mapInfo.volcanoName} luar area.`);
                return; // <-- Skrip berhenti, alarm tidak akan bunyi
            }
        }

        // --- Jika lolos kedua filter di atas, barulah bunyikan alarm! ---
        const logMsg = `BARU: Hash baru terdeteksi! Menampilkan notifikasi ${mapInfo ? mapInfo.volcanoName : ''}`;
        console.log(`[VAA] ${logMsg}`);
        updateDebugStatus(logMsg);
		saveToArchive(data); // <--- TAMBAHKAN BARIS utk ARSIP VAA
...
document.addEventListener('DOMContentLoaded', function() {
    map.on('overlayadd', function(e) {
        if (e.name === 'VA Advisory') {
			...
			archiveControl.addTo(map); // <--- TAMPILKAN KOTAK ARSIP
            renderArchiveList();       // <--- RENDER ISINYA
            isFirstCheck = true;
            checkForNewVAA();
    ...
    map.on('overlayremove', function(e) {
        if (e.name === 'VA Advisory') {
            ...
            vaAdvisoryLayer.clearLayers();
            vaaPolygonPreviewLayer.clearLayers();
			map.removeControl(archiveControl); // <--- SEMBUNYIKAN KOTAK ARSIP
            const alertSound = document.getElementById('vaa-alert-sound');
            if (alertSound) { alertSound.pause(); alertSound.currentTime = 0; }
